...
# Setup mock database before importing the app or models
mock_client = AsyncMongoMockClient()
mock_db = mock_client["test_db"]

# Patch database module level attributes
...
async def override_get_current_admin():
    return {"email": "admin@mca.com", "role": "admin"}
...
def setup_and_teardown():
    # Setup: Initialize Beanie database and clear collections before each test
    from core.database_init import initialize_database
    import asyncio
    
    async def init_and_clear():
        await initialize_database(custom_db=database.db)
        database.db.delegate["members"].delete_many({})
        database.db.delegate["templates"].delete_many({})
        database.db.delegate["department_events"].delete_many({})
        database.db.delegate["achievements"].delete_many({})
    ...
    loop.run_until_complete(init_and_clear())
    yield
...
def test_jwt_encode_decode():
    ...
    payload = {"email": "test@mca.com", "role": "admin"}
    token = encode_jwt(payload, expires_in=10)
    decoded = decode_jwt(token)
    assert decoded is not None
    assert decoded["email"] == "test@mca.com"
...
def test_jwt_expired():
    from core.auth import encode_jwt, decode_jwt
    payload = {"email": "test@mca.com", "role": "admin"}
    token = encode_jwt(payload, expires_in=-1)
    decoded = decode_jwt(token)
...
def test_bulk_member_import():
    ...
    response = client.post("/api/members/bulk", json=members_data)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
    assert data[0]["name"] == "Bulk Member 1"
    assert data[1]["name"] == "Bulk Member 2"
