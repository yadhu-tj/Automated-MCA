import { Member, Achievement, DepartmentEvent, Template } from '../types';
...
export const api = {
  ...
  updateTemplate: async (id: string, templateData: Omit<Template, 'id'>): Promise<Template> => {
  ...
  importMembersBulk: async (membersData: Omit<Member, 'id'>[]): Promise<Member[]> => {
    const response = await fetch(`${API_BASE_URL}/members/bulk`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.detail || 'Failed to import members');
    }
    return response.json();
  }
};
