...
import { ApprovalStatus, Member, Achievement, DepartmentEvent, Template } from '../types';
import { BookOpen, Award, Sparkles } from 'lucide-react';

import { HeroSection } from '../components/public/HeroSection';
import { BirthdaysWidget } from '../components/public/BirthdaysWidget';
import { FestivalsWidget } from '../components/public/FestivalsWidget';
...
export const PublicHome: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAdditionalData = async () => {
      try {
        ...
        const savedTemplates = await api.getTemplates();
  ...
  const birthdaysRef = useFadeInRef();
  const festivalsRef = useFadeInRef();
  const calendarRef = useFadeInRef();
  const achievementsRef = useFadeInRef();
  ...
  return (
    <div className="pb-16">
      ...
      <div ref={birthdaysRef}>
        <BirthdaysWidget todaysBirthdays={todaysBirthdays} templates={templates} />
      </div>

      {/* ── Upcoming Festivals Section ── */}
      <div ref={festivalsRef}>
        <FestivalsWidget upcomingEvents={upcomingEvents} templates={templates} />
      </div>
      ...
      <div ref={achievementsRef} className="mt-8">
        <AchievementWall
