  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (newMembers.length > 0) {
        const imported = await api.importMembersBulk(newMembers);
        setMembers(prev => {
          const existingEmails = new Set(prev.map(m => m.email));
          const filteredImported = imported.filter(m => !existingEmails.has(m.email));
